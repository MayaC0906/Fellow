...
export function OurSiri({ isSiriOpen, setSiriOpen }) {
    ...
    useEffect(() => {
        ...
        if (listening && transcript) {
            // Set a timeout to process the transcript after a pause
            timeoutId = setTimeout(handleInterimResult, 1000);
            lastResultTimeRef.current = Date.now(); // Update time on new transcript
        }
    ...
    function correctWord(word) {
            ...
            function getAction() {
              ...
              function extractTitle(transcript) {
                const titledKeyword = "titled";
                const indexOfTitled = transcript.indexOf(titledKeyword);
                if (indexOfTitled !== -1) {
                  // Extract everything after the keyword "titled"
                  return transcript.slice(indexOfTitled + titledKeyword.length).trim();
                }
                return null; // Return null or some default title if "titled" is not found
              }
              ...
              function extractGroupNameAndTaskTitle(transcript) {
                ...
                if (indexOfIn !== -1 && indexOfTitled !== -1) {
                  // Extract the group name found between "in" and ", titled"
                  const groupName = transcript.slice(indexOfIn + inKeyword.length, indexOfTitled).trim();
                  // Extract the task title found after ", titled"
                  const taskTitle = transcript.slice(indexOfTitled + titledKeyword.length).trim();
                  return { groupName, taskTitle };
                }
                return null; // Return null or some defaults if the required keywords are not found
              }
              // Dummy function to extract task title
              function extractTaskTitle(transcript) {
                // TODO: Implement the logic to extract the task title from the transcript
                // This is placeholder code and needs to be replaced with actual logic
